...
export default function ParkingPage() {
    ...
    const getParkingRecord = async (cardItem: any) => {
        const response = await fetch(
            `/api/parking/wx/park/car/parking/list?pageNum=1&pageSize=999&bindingId=${cardItem.bindingId}&carId=${cardItem.carId}&geoId=`,
            {
                method: "GET",
                headers,
            }
    ...
    return (
        <div>
            <div
                onClick={async () => {
                    const resp = await fetch('/api/parking/wx/park/car/pay', {
